...
//	  obj.add(p.getName()); obj.add(p.getPrice()); obj.add(p.getCategory());
//	  obj.add(p.getPnum()); obj.add(p.getDescription());
//	  //2.创建sql语句，并拼接sql String
//	  sql = "update products " +
//	  "set  name=?,price=?,category=?,pnum=?,description=? ";
//	  //判断是否有图片 
//		if(p.getImgurl() != null && p.getImgurl().trim().length() > 0) { sql +=
//	  " ,imgurl=?"; obj.add(p.getImgurl()); } sql += " where id=?";
//	  obj.add(p.getId()); System.out.println(sql); System.out.println(obj);
//	  //3.创建QueryRunner对象 QueryRunner runner = new
...
//	  //删除订单时，修改商品数量 
//		public void updateProductNum(List<OrderItem> items) throws
//	  SQLException {
//	  
//	  String sql = "update products set pnum=pnum+? where id=?"; 
//	  QueryRunner runner
...
//	  Object[][] params = new Object[items.size()][2];
//	  
//	  for (int i = 0; i < params.length; i++) { params[i][0] =
//	  items.get(i).getBuynum(); params[i][1] = items.get(i).getP().getId(); }
//	  
//	  runner.batch(DataSourceUtils.getConnection(), sql, params); }
